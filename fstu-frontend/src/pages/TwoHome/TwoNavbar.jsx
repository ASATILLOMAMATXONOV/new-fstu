...
import {
  ...
  Assignment, Menu as MenuIcon, Close, ExpandLess, ExpandMore
...
const megaMenuData = {
  UNIVERSITET: [
    ...
    {
      columnName: "ALOQA",
      items: [{ name: "Bog'lanish", icon: <Phone />, url: "/twocontact" }],
    },
  ],
  FAOLIYAT: [
    {
      columnName: "MA’NAVIY FAOLIYAT",
      items: [
    ...
    {
      columnName: "O'QUV FAOLIYAT",
      items: [
        { name: "Bakalavriat", icon: <School />, url: "/twoactivity" },
        { name: "Magistratura", icon: <School />, url: "/twopages" },
        { name: "NORMATIV XUJJATLAR", icon: <MenuBook />, url: "/twopages" },
      ],
    },
    {
      columnName: "ILMIY INNOVATSION FAOLIYAT",
      items: [
    ...
    {
      columnName: "MOLIYAVIY KO'RSATKICHLAR",
      items: [
...
const TwoNavbar = () => {
  ...
  // trigger faqat shadow uchun ishlaydi (layout o'zgarmaydi)
  ...
  // ✅ DOIMIY O'LCHAMLAR (scroll'da o'zgarmaydi)
  const DESKTOP_LOGO_H = 55;
  const DESKTOP_TOOLBAR_PY = 1.5;
    ...
    <>
      <AppBar
        ...
        sx={{
          ...
          transition: "box-shadow 0.25s ease", // ✅ faqat shadow
        }}
      >
        {/* --- TOP BAR (Desktop) --- */}
        {!isMobile && (
        ...
        <Container maxWidth="xl">
          <Toolbar
            disableGutters
            sx={{
              py: isMobile ? 1 : DESKTOP_TOOLBAR_PY, // ✅ doimiy
          ...
          >
            <Box component="a" href="/" sx={{ display: "flex", alignItems: "center" }}>
              <img
                ...
                alt="Logo"
                style={{
                  height: isMobile ? 40 : DESKTOP_LOGO_H, // ✅ doimiy
                  display: "block",
                }}
              />
            </Box>
                      ...
                      >
                        <Container maxWidth="xl" sx={{ py: 0, display: "flex" }}>
                          <Box sx={{ width: "300px", bgcolor: alpha(fstuBlue, 0.03), p: 6, borderRight: `1px solid ${alpha(fstuBlue, 0.08)}` }}>
                            <Typography variant="h4" sx={{ fontWeight: 600, fontSize: "1.25rem", color: fstuBlue, textTransform: "uppercase" }}>
                              {item.name}
                            </Typography>
                            <Box sx={{ width: "50px", height: "4px", bgcolor: fstuBlue, mb: 3 }} />
                            <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.7 }}>
                              Farg'ona Politexnika Instituti {item.name.toLowerCase()} xizmatlari.
                            </Typography>
                          </Box>
                          ...
                          <Box sx={{ flex: 1, p: 3, display: "grid", flexWrap: "wrap", gridTemplateColumns: "repeat(5, 1fr)", gap: 4 }}>
                            {megaMenuData[item.name].map((col, idx) => (
                              <Box key={idx}>
                                <Typography sx={{ fontWeight: 800, color: fstuBlue, fontSize: "11px", mb: 2.5, letterSpacing: 1.2, textTransform: "uppercase" }}>
                                  {col.columnName}
                                </Typography>
                                <Stack spacing={1.8}>
                                  {col.items.map((link) => (
                                    ...
                                    >
                                      <Box sx={{ color: fstuBlue, opacity: 0.8, display: "flex" }}>
                                        {React.cloneElement(link.icon, { sx: { fontSize: 18 } })}
                                      </Box>
                                      <Typography sx={{ fontWeight: 600, textTransform: "uppercase", fontSize: "0.75rem" }}>
                                        {link.name}
                                      </Typography>
                                    </Stack>
                                  ))}
      ...
      >
        <Box sx={{ p: 2 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
            <img src={LogoImg} alt="Logo" style={{ height: 35, display: "block" }} />
                    ...
                    <Box sx={{ bgcolor: alpha(fstuBlue, 0.02), pl: 2, pb: 2 }}>
                      {megaMenuData[item.name].map((col) => (
                        <Box key={col.columnName} sx={{ mt: 3 }}>
                          <Typography variant="caption" sx={{ fontWeight: 800, color: fstuBlue, ml: 2, textTransform: "uppercase" }}>
                            {col.columnName}
                          </Typography>
                          {col.items.map((sub) => (
                            <ListItem button key={sub.name} sx={{ py: 1, mt: 0.5 }} component="a" href={sub.url}>
          ...
          <Box sx={{ mt: 4, textAlign: "center" }}>
            ...
            <Button fullWidth variant="contained" sx={{ bgcolor: fstuBlue, py: 1.5, fontWeight: 700 }}>
              LOGIN
            </Button>
          </Box>
        </Box>
